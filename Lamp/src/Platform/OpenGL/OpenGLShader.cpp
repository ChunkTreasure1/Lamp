...
OpenGLShader::OpenGLShader(const std::string &vertexPath,
  ...
  while (std::getline(fragmentFile, line)) {
    ...
    if (line.find("Name") != std::string::npos &&
      ...
      std::string s = line.substr(line.find_first_of(":") + 1,
  ...
  if (!success) {
    ...
    LP_ERROR("Vertex shader compilation failed: " + std::string(infoLog) +
             ". At: " + vertexPath);
  }
  ...
  if (!success) {
    ...
    LP_ERROR("Fragment shader compilation failed: " + std::string(infoLog) +
             ". At: " + fragmentPath);
  }
  ...
  if (geoPath != "") {
    ...
    if (!success) {
      glGetShaderInfoLog(fragment, 512, NULL, infoLog);
      LP_ERROR("Geometry shader compilation failed: " + std::string(infoLog) +
               ". At: " + fragmentPath);
    }
  }
  ...
  if (!success) {
    ...
    LP_ERROR("Shader link failed: " + std::string(infoLog) +
             ". At: " + m_Specifications.Name);
  }
...
void OpenGLShader::UploadFloat3(const std::string &name,
                                const glm::vec3 &value) const {
  glUniform3f(glGetUniformLocation(m_RendererID, name.c_str()), value.x,
              value.y, value.z);
}

void OpenGLShader::UploadFloat4(const std::string &name,
                                const glm::vec4 &value) const {
  glUniform4f(glGetUniformLocation(m_RendererID, name.c_str()), value.x,
...
void OpenGLShader::UploadIntArray(const std::string &name, int *values,
