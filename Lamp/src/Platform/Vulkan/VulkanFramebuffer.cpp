...
namespace Lamp
{
		...
		: m_specification(spec)
	{
		if (!m_specification.swapchainTarget)
		{
			...
			for (auto& attachmentSpec : m_specification.attachments.Attachments)
			{
				...
				{
					if (!Utils::IsDepthFormat(m_specification.existingImages[attachmentIndex]->GetSpecification().format))
					{
						m_attachmentImages.emplace_back();
					}
				}
				else if (Utils::IsDepthFormat(attachmentSpec.textureFormat))
				{
	...
	void VulkanFramebuffer::Copy(uint32_t rendererId, const glm::vec2& size, bool depth)
	{
	}
	...
	void VulkanFramebuffer::Invalidate()
	{
		...
		for (auto attachmentSpec : m_specification.attachments.Attachments)
		{
			if (Utils::IsDepthFormat(attachmentSpec.textureFormat))
			{
				...
				{
					auto vulkanImage = std::reinterpret_pointer_cast<VulkanImage2D>(m_depthAttachmentImage);
			...
			else
			{
				...
				else
				{
					if (createImages)
					{
						...
						colorAttachment = m_attachmentImages.emplace_back(std::reinterpret_pointer_cast<VulkanImage2D>(Image2D::Create(spec)));
					}
					else
					{
				...
				attachmentDescripton.loadOp = existingImage ? VK_ATTACHMENT_LOAD_OP_LOAD : Utility::LampLoadToVulkanLoadOp(attachmentSpec.clearMode); // TODO: add other ops
		...
		{
			Ref<VulkanImage2D> image = std::reinterpret_pointer_cast<VulkanImage2D>(m_attachmentImages[i]);
		...
		if (m_depthAttachmentImage)
		{
			Ref<VulkanImage2D> image = std::reinterpret_pointer_cast<VulkanImage2D>(m_depthAttachmentImage);
	...
	const uint32_t VulkanFramebuffer::GetDepthAttachmentID()
	{
		return uint32_t();
	}

	const uint32_t VulkanFramebuffer::GetRendererID()
	{
		return uint32_t();
	}

	void VulkanFramebuffer::BindColorAttachment(uint32_t id, uint32_t i)
	{
	}

	void VulkanFramebuffer::BindDepthAttachment(uint32_t id)
	{
	}

	FramebufferSpecification& VulkanFramebuffer::GetSpecification()
	{
