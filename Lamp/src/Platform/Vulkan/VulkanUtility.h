...
namespace Utility
{
	...
	{
		auto device = Lamp::VulkanContext::GetCurrentDevice();
		auto swapchain = Lamp::Application::Get().GetWindow().GetSwapchain();
	...
	static void TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, Ref<Lamp::VulkanDevice> device)
	{
		VkCommandBuffer commandBuffer = device->GetCommandBuffer(true);
	...
	{
		auto device = Lamp::VulkanContext::GetCurrentDevice();
		return CreateImageView(image, format, aspect, device.get());
	}
}