...
namespace Lamp
{
	...
	TextureCube::TextureCube(const std::filesystem::path& path)
	...
	void TextureCube::SetData(Ref<Image2D> image, uint32_t face, uint32_t mip)
	{
		auto device = VulkanContext::GetCurrentDevice();
		auto commandBuffer = device->GetCommandBuffer(true);

		Utility::TransitionImageLayout(image->GetHandle(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
		...
		vkCmdCopyImage(commandBuffer, image->GetHandle(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);

		device->FlushCommandBuffer(commandBuffer);

		Utility::TransitionImageLayout(image->GetHandle(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
	}
