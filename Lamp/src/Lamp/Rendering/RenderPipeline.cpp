...
namespace Lamp
{
	...
	{
		vkCmdBindDescriptorSets(commandBuffer->GetCurrentCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, m_layout, startSet, descriptorSets.size(), descriptorSets.data(), 0, nullptr);
	}

	void RenderPipeline::BindDescriptorSet(Ref<CommandBuffer> commandBuffer, VkDescriptorSet descriptorSet, uint32_t set) const
	{
		vkCmdBindDescriptorSets(commandBuffer->GetCurrentCommandBuffer(), VK_PIPELINE_BIND_POINT_GRAPHICS, m_layout, set, 1, &descriptorSet, 0, nullptr);
	}
	...
	void RenderPipeline::SetTexture(Ref<Texture2D> texture, uint32_t binding, uint32_t set, uint32_t index)
	{
		...
		auto& shaderDescriptorSets = m_specification.shader->GetDescriptorSets();
