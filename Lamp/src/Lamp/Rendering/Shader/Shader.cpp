...
namespace Lamp
{
	...
	void Shader::LoadAndCreateShaders(const std::unordered_map<VkShaderStageFlagBits, std::vector<uint32_t>>& outShaderData)
	{
			auto device = VulkanContext::GetCurrentDevice()->GetHandle();
			m_pipelineShaderStageInfos.clear();

			for (auto [stage, data] : outShaderData)
			{
	...
	void Shader::ReflectAllShaderStages(const std::unordered_map<VkShaderStageFlagBits, std::vector<uint32_t>>& outShaderData)
	{
		m_resources.clear();
