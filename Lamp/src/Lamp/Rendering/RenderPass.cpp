...
RenderPass::RenderPass(Ref<FrameBuffer> &frameBuffer,
                       const RenderPassInfo &passInfo,
                       std::vector<RenderFunc> extraRenders)
    : m_FrameBuffer(frameBuffer), m_PassInfo(passInfo),
      m_ExtraRenders(extraRenders) {}
...
bool RenderPassManager::RemovePass(uint32_t id) {
  auto it = std::remove_if(m_RenderPasses.begin(), m_RenderPasses.end(),
                           [id](auto pass) { return pass->GetID() == id; });
