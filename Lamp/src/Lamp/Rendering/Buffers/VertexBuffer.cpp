...
namespace Lamp
{
	...
	/////Vertex Buffer/////
	VertexBuffer::VertexBuffer(const std::vector<Vertex>& vertices, uint32_t aSize)
	{
	...
	VertexBuffer::VertexBuffer(const std::vector<float>& vertices, uint32_t size)
	{
	...
	void VertexBuffer::Bind(Ref<CommandBuffer> commandBuffer) const
	{
		...
		vkCmdBindVertexBuffers(commandBuffer->GetCurrentCommandBuffer(), 0, 1, buffers, offsets);
	}
	...
	void VertexBuffer::SetData(const void* data, uint32_t aSize)
	{
		auto device = VulkanContext::GetCurrentDevice();
	...
	{
		vkCmdBindIndexBuffer(commandBuffer->GetCurrentCommandBuffer(), m_buffer, 0, VK_INDEX_TYPE_UINT32);
	}
