...
struct Renderer2DStorage {
  ...
  Ref<Texture2D> *TextureSlots{
      new Ref<Texture2D>[RenderCommand::GetCapabilities().MaxTextureSlots]};
...
void Renderer2D::Initialize() {
  ...
  s_pData->pQuadVertexBuffer =
      VertexBuffer::Create(s_pData->MaxVertices * sizeof(QuadVertex));
  ...
  for (uint32_t i = 0; i < RenderCommand::GetCapabilities().MaxTextureSlots;
       i++) {
    samplers[i] = i;
  }

  s_pData->pTextureShader = Shader::Create("engine/shaders/2d/Texture.vert",
                                           "engine/shaders/2d/Texture.frag");
  s_pData->pTextureShader->Bind();
  s_pData->pTextureShader->UploadIntArray(
      "u_Textures", samplers, RenderCommand::GetCapabilities().MaxTextureSlots);
...
void Renderer2D::Begin(const Ref<CameraBase> &camera) {
  ...
  s_pData->pTextureShader->Bind();
  s_pData->pTextureShader->UploadMat4("u_ViewProjection",
...
void Renderer2D::End() {
  uint32_t dataSize = (uint8_t *)s_pData->QuadVertexBufferPtr -
                      (uint8_t *)s_pData->QuadVertexBufferBase;
  s_pData->pQuadVertexBuffer->SetData(s_pData->QuadVertexBufferBase, dataSize);

  dataSize = (uint8_t *)s_pData->pLineVertexBufferPtr -
             (uint8_t *)s_pData->pLineVertexBufferBase;
...
void Renderer2D::Flush() {
  ...
  // Draw
  RenderCommand::DrawIndexed(s_pData->pQuadVertexArray,
                             s_pData->QuadIndexCount);
  s_pData->Stats.DrawCalls++;

  RenderCommand::DrawIndexedLines(s_pData->pLineVertexArray,
...
void Renderer2D::DrawQuad(const glm::mat4 &tm, const Ref<Texture2D> &texture,
...
void Renderer2D::DrawQuad(const glm::vec3 &pos, const glm::vec2 &scale,
  ...
  s_pData->QuadVertexBufferPtr->Position =
      transform * s_pData->QuadVertexPositions[0];
  s_pData->QuadVertexBufferPtr->Color = color;
  s_pData->QuadVertexBufferPtr->TexCoord = {0.f, 0.f};
  s_pData->QuadVertexBufferPtr->TexIndex = texIndex;
  s_pData->QuadVertexBufferPtr++;
  ...
  s_pData->QuadVertexBufferPtr->Position =
      transform * s_pData->QuadVertexPositions[1];
  s_pData->QuadVertexBufferPtr->Color = color;
  s_pData->QuadVertexBufferPtr->TexCoord = {1.f, 0.f};
  s_pData->QuadVertexBufferPtr->TexIndex = texIndex;
  s_pData->QuadVertexBufferPtr++;
  ...
  s_pData->QuadVertexBufferPtr->Position =
      transform * s_pData->QuadVertexPositions[2];
  s_pData->QuadVertexBufferPtr->Color = color;
  s_pData->QuadVertexBufferPtr->TexCoord = {1.f, 1.f};
  s_pData->QuadVertexBufferPtr->TexIndex = texIndex;
  s_pData->QuadVertexBufferPtr++;
  ...
  s_pData->QuadVertexBufferPtr->Position =
      transform * s_pData->QuadVertexPositions[3];
  s_pData->QuadVertexBufferPtr->Color = color;
  s_pData->QuadVertexBufferPtr->TexCoord = {0.f, 1.f};
  s_pData->QuadVertexBufferPtr->TexIndex = texIndex;
  s_pData->QuadVertexBufferPtr++;
...
Renderer2D::Statistics Renderer2D::GetStats() { return s_pData->Stats; }
