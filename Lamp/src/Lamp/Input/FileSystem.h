...
namespace Lamp {
enum class FileType { Empty = 0, Texture, Text, Level, Brush };
...
public:
  ...
  static void GetAllMaterialFiles(std::vector<std::string> &folders,
                                  std::vector<std::string> &files);
  static bool ContainsFolder(const std::string &path);
  static void PrintFoldersAndFiles(std::vector<std::string> &files,
                                   int startId = -1);
  static void PrintLevelFiles(std::vector<std::string> &files,
                              int startID = -1);
