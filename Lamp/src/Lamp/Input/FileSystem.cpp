...
FileSystem::GetMaterialFolders(const std::filesystem::path &path) {
...
std::vector<std::string> FileSystem::GeFilesOfType(const std::string &path,
...
void FileSystem::GetAllFilesOfType(std::vector<std::string> &files,
                                   const std::string &type,
...
bool FileSystem::WriteBytes(const std::filesystem::path &path,
                            const Buffer &buffer) {
  ...
  for (auto &folder : folders) {
    ...
    for (const auto &file : files) {
      ...
      if (ImGui::BeginDragDropSource()) {
        ...
        ImGui::SetDragDropPayload("BRUSH_ITEM", path,
                                  sizeof(char) * (file.length() + 1),
                                  ImGuiCond_Once);
