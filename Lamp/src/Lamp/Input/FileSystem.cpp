...
void FileSystem::GetAllMaterialFiles(std::vector<std::string> &folders,
                                     std::vector<std::string> &files) {
  if (folders.size() == 0) {
    return;
  }
...
// Prints all the available folders and files (CALL ONLY WHEN IMGUI CONTEXT
// EXISTS OR IT WILL CRASH!)
...
// Prints all the available folders and files (CALL ONLY WHEN IMGUI CONTEXT
// EXISTS OR IT WILL CRASH!)
void FileSystem::PrintLevelFiles(std::vector<std::string> &folders,
                                 int startID) {
  if (folders.size() == 0) {
    return;
  }
  ...
  for (int i = 0; i < folders.size(); i++) {
    ...
    std::vector<std::string> files =
        Lamp::FileSystem::GetLevelFiles(folders[i]);
        ...
        if (ImGui::MenuItem(p.c_str())) {
          ...
          if (f.GetFileType() == FileType::Level) {
            Lamp::LevelSystem::SaveLevel(
                "engine/levels/" +
                    Lamp::LevelSystem::GetCurrentLevel()->GetName() + ".level",
...
void FileSystem::PrintBrushes(std::vector<std::string> &folders, int startID) {
  ...
  for (int i = 0; i < folders.size(); i++) {
    ...
    std::vector<std::string> files =
        Lamp::FileSystem::GetBrushFiles(folders[i]);
