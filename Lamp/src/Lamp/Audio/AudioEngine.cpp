...
AudioImplementation::AudioImplementation() {
  ...
  AudioEngine::ErrorCheck(StudioSystem->initialize(
      512, FMOD_STUDIO_INIT_LIVEUPDATE, FMOD_INIT_PROFILE_ENABLE, nullptr));
...
void AudioEngine::Update() { g_AudioImplementation->Update(); }

void AudioEngine::Shutdown() { delete g_AudioImplementation; }
...
void AudioEngine::LoadBanksFromDefault() {
  std::vector<std::string> files =
      FileSystem::GetFiles(std::string(DEFAULT_BANK_PATH));
  if (files.size() > 0) {
    for (auto &file : files) {
...
void AudioEngine::LoadBank(const std::string &name,
  ...
  if (auto it = g_AudioImplementation->Banks.find(name);
      it != g_AudioImplementation->Banks.end()) {
    return;
  }

  FMOD::Studio::Bank *bank;
  AudioEngine::ErrorCheck(g_AudioImplementation->StudioSystem->loadBankFile(
      name.c_str(), flags, &bank));
  if (bank) {
...
void AudioEngine::LoadEvent(const std::string &name) {
  if (auto it = g_AudioImplementation->Events.find(name);
      it != g_AudioImplementation->Events.end()) {
    return;
  }

  FMOD::Studio::EventDescription *eventDesc = nullptr;
  AudioEngine::ErrorCheck(
      g_AudioImplementation->StudioSystem->getEvent(name.c_str(), &eventDesc));
...
void AudioEngine::LoadSound(const std::string &name, bool threeD, bool looping,
                            bool stream) {
  if (auto it = g_AudioImplementation->Sounds.find(name);
      it != g_AudioImplementation->Sounds.end()) {
    return;
  }
  ...
  AudioEngine::ErrorCheck(g_AudioImplementation->System->createSound(
      name.c_str(), mode, nullptr, &sound));
  if (sound) {
...
void AudioEngine::Set3DListenerAndOrientation(const glm::vec3 &pos,
                                              float volume) {}

int AudioEngine::PlaySound(const std::string &name, const glm::vec3 &pos,
                           float volume) {
  ...
  AudioEngine::ErrorCheck(g_AudioImplementation->System->playSound(
      it->second, nullptr, true, &channel));
  if (channel) {
    FMOD_MODE currMode;
...
void AudioEngine::StopChannel(int channel) {
  if (auto it = g_AudioImplementation->Channels.find(channel);
      it != g_AudioImplementation->Channels.end()) {
...
void AudioEngine::GetEventParameter(const std::string &name,
                                    const std::string &eventParams,
                                    float *parameter) {
  if (auto it = g_AudioImplementation->Events.find(name);
      it != g_AudioImplementation->Events.end()) {
    AudioEngine::ErrorCheck(
        it->second->getParameterByName(eventParams.c_str(), parameter));
  }
}

void AudioEngine::SetEventParameter(const std::string &name,
...
void AudioEngine::SetListenerAttributes(const Listener &listener,
                                        const ListenerAttributes &attr) {
  FMOD_3D_ATTRIBUTES attributes;
  ...
  AudioEngine::ErrorCheck(
      g_AudioImplementation->StudioSystem->setListenerAttributes(
          listener.ListenerID, &attributes, NULL));
}
...
float AudioEngine::DBToVolume(float db) { return std::powf(10.f, 0.05f * db); }
