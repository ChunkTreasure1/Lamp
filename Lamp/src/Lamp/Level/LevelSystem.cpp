...
Ref<Level> LevelSystem::LoadLevel(const std::string &path) {
  ...
  std::vector<char> buffer((std::istreambuf_iterator<char>(levelFile)),
  ...
  if (rapidxml::xml_node<> *pLevelEnv =
          pRootNode->first_node("LevelEnvironment")) {
  ...
  if (rapidxml::xml_node<> *pLayers = pRootNode->first_node("Layers")) {
    ObjectLayerManager::Get()->SetLayers(
        LoadLayers(pLayers, ObjectLayerManager::Get()));
  }
  ...
  if (rapidxml::xml_node<> *pEntities = pRootNode->first_node("Entities")) {
    EntityManager::Get()->SetEntities(
        LoadEntities(pEntities, EntityManager::Get()));
  }
...
bool LevelSystem::SaveLevel(const std::string &path, Ref<Level> &level) {
  ...
  rapidxml::xml_node<> *pRoot =
      doc.allocate_node(rapidxml::node_element, "Level");
  pRoot->append_attribute(
      doc.allocate_attribute("name", level->GetName().c_str()));

  /////Brushes/////
  rapidxml::xml_node<> *pBrushes =
      doc.allocate_node(rapidxml::node_element, "Brushes");
  for (auto &brush : BrushManager::Get()->GetBrushes()) {
    rapidxml::xml_node<> *child =
        doc.allocate_node(rapidxml::node_element, "Brush");

    child->append_attribute(doc.allocate_attribute(
        "lgfPath", brush->GetModel()->GetLGFPath().c_str()));
    ...
    char *pLayer =
        doc.allocate_string(ToString((int)brush->GetLayerID()).c_str());
  ...
  /////Layers//////
  rapidxml::xml_node<> *pLayers =
      doc.allocate_node(rapidxml::node_element, "Layers");
  for (auto &layer : ObjectLayerManager::Get()->GetLayers()) {
    if (layer.ID == 0)
      continue;

    rapidxml::xml_node<> *child =
        doc.allocate_node(rapidxml::node_element, "Layer");
    ...
    char *pDestroyable =
        doc.allocate_string(ToString(layer.IsDestroyable).c_str());
    child->append_attribute(
        doc.allocate_attribute("destroyable", pDestroyable));
  ...
  ////Entities/////
  rapidxml::xml_node<> *pEntities =
      doc.allocate_node(rapidxml::node_element, "Entities");
  for (auto &entity : EntityManager::Get()->GetEntities()) {
    if (!entity->GetSaveable()) {
      continue;
    }

    rapidxml::xml_node<> *ent =
    ...
    for (auto component : entity->GetComponents()) {
      rapidxml::xml_node<> *comp =
          doc.allocate_node(rapidxml::node_element, "Component");
      ...
      for (auto prop : component->GetComponentProperties().GetProperties()) {
        rapidxml::xml_node<> *param =
            doc.allocate_node(rapidxml::node_element, "Param");

        char *pParamName = doc.allocate_string(prop.Name.c_str());
        param->append_attribute(doc.allocate_attribute("name", pParamName));

        char *pType = doc.allocate_string(
            ComponentProperties::GetStringFromType(prop.PropertyType).c_str());
        param->append_attribute(doc.allocate_attribute("type", pType));

        switch (prop.PropertyType) {
        case Lamp::PropertyType::String: {
          char *pValue = doc.allocate_string(
              static_cast<std::string *>(prop.Value)->c_str());
          param->append_attribute(doc.allocate_attribute("value", pValue));
          break;
        }
        case Lamp::PropertyType::Bool: {
          char *pValue = doc.allocate_string(
              ToString(*static_cast<bool *>(prop.Value)).c_str());
          param->append_attribute(doc.allocate_attribute("value", pValue));
          break;
        }
        case Lamp::PropertyType::Int: {
          char *pValue = doc.allocate_string(
              ToString(*static_cast<int *>(prop.Value)).c_str());
          param->append_attribute(doc.allocate_attribute("value", pValue));
          break;
        }
        case Lamp::PropertyType::Float: {
          char *pValue = doc.allocate_string(
              ToString(*static_cast<float *>(prop.Value)).c_str());
          param->append_attribute(doc.allocate_attribute("value", pValue));
          break;
        }
        case Lamp::PropertyType::Float2: {
          char *pValue = doc.allocate_string(
              ToString(*static_cast<glm::vec2 *>(prop.Value)).c_str());
          param->append_attribute(doc.allocate_attribute("value", pValue));
          break;
        }
        case Lamp::PropertyType::Float3: {
          char *pValue = doc.allocate_string(
              ToString(*static_cast<glm::vec3 *>(prop.Value)).c_str());
          param->append_attribute(doc.allocate_attribute("value", pValue));
          break;
        }
        case Lamp::PropertyType::Float4: {
          char *pValue = doc.allocate_string(
              ToString(*static_cast<glm::vec4 *>(prop.Value)).c_str());
          param->append_attribute(doc.allocate_attribute("value", pValue));
          break;
        }
  ...
  /////Level environment/////
  rapidxml::xml_node<> *pLevelEnv =
      doc.allocate_node(rapidxml::node_element, "LevelEnvironment");

  rapidxml::xml_node<> *globalAmbient =
      doc.allocate_node(rapidxml::node_element, "GlobalAmbient");
  char *pGA = doc.allocate_string(
      ToString(level->GetEnvironment().GlobalAmbient).c_str());
  globalAmbient->append_attribute(doc.allocate_attribute("value", pGA));

  rapidxml::xml_node<> *cameraValues =
      doc.allocate_node(rapidxml::node_element, "CameraValues");
  char *pCP = doc.allocate_string(
      ToString(level->GetEnvironment().CameraPosition).c_str());
  cameraValues->append_attribute(doc.allocate_attribute("position", pCP));

  char *pCR = doc.allocate_string(
      ToString(level->GetEnvironment().CameraRotation).c_str());
  cameraValues->append_attribute(doc.allocate_attribute("rotation", pCR));

  char *pCF =
      doc.allocate_string(ToString(level->GetEnvironment().CameraFOV).c_str());
...
std::vector<ObjectLayer>
LevelSystem::LoadLayers(rapidxml::xml_node<> *pNode,
                        ObjectLayerManager *objLayerManager) {
  std::vector<ObjectLayer> layers;

  for (rapidxml::xml_node<> *pLayer = pNode->first_node("Layer"); pLayer;
...
std::vector<Brush *> LevelSystem::LoadBrushes(rapidxml::xml_node<> *pNode,
                                              BrushManager *brushManager) {
  std::vector<Brush *> pBrushes;

  for (rapidxml::xml_node<> *pBrush = pNode->first_node("Brush"); pBrush;
       pBrush = pBrush->next_sibling()) {
    ...
    pBrushes.push_back(
        brushManager->Create(path, pos, rot, scale, layerID, name));
  }

  return pBrushes;
}

std::vector<Entity *> LevelSystem::LoadEntities(rapidxml::xml_node<> *pNode,
                                                EntityManager *entityManager) {
  std::vector<Entity *> pEntities;

  for (rapidxml::xml_node<> *pEntity = pNode->first_node("Entity"); pEntity;
       pEntity = pEntity->next_sibling()) {
    ...
    for (rapidxml::xml_node<> *pComponent = pEntity->first_node("Component");
         pComponent; pComponent = pComponent->next_sibling()) {
      ...
      for (rapidxml::xml_node<> *pParam = pComponent->first_node("param");
           pParam; pParam = pParam->next_sibling()) {
        auto paramName = pParam->first_attribute("name")->value();
        auto type = ComponentProperties::GetTypeFromString(
            pParam->first_attribute("type")->value());
...
std::string LevelSystem::ToString(const glm::vec4 &var) {
  std::string str(std::to_string(var.x) + "," + std::to_string(var.y) + "," +
                  std::to_string(var.z) + "," + std::to_string(var.w));
  return str;
}
//////////////////////////
} // namespace Lamp