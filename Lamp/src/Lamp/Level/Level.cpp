...
namespace Lamp {
RenderUtils::~RenderUtils() { m_PointLights.clear(); }
...
void RenderUtils::RegisterMeshComponent(uint32_t id,
                                        Lamp::MeshComponent *mesh) {
...
Level::Level(const Level &level) {
  for (auto &brush : level.m_Brushes) {
    m_Brushes.emplace(
        std::make_pair(brush.first, Brush::Duplicate(brush.second, false)));
  }

  for (auto &entity : level.m_Entities) {
    std::pair pair =
        std::make_pair(entity.first, Entity::Duplicate(entity.second, false));
...
void Level::OnEvent(Event &e) {
  ...
  dispatcher.Dispatch<EditorViewportSizeChangedEvent>(
      LP_BIND_EVENT_FN(Level::OnViewportResize));
}
...
void Level::OnSimulationEnd() { Physics::DestroyScene(); }
...
void Level::AddLayer(const ObjectLayer &layer) { m_Layers.push_back(layer); }
...
void Level::RemoveLayer(uint32_t id) {
  ...
  if (auto it =
          std::find_if(m_Layers.begin(), m_Layers.end(),
                       [id](ObjectLayer &layer) { return id == layer.ID; });
      it != m_Layers.end()) {
    m_Layers.erase(it);
  }
}

void Level::MoveObjectToLayer(uint32_t currLayer, uint32_t newLayer,
                              uint32_t objId) {
  ...
  if (auto it = std::find(currObjLayer->Objects.begin(),
                          currObjLayer->Objects.end(), obj);
      it != currObjLayer->Objects.end()) {
...
void Level::RemoveFromLayer(Object *obj) {
  for (auto &layer : m_Layers) {
    if (auto it = std::find(layer.Objects.begin(), layer.Objects.end(), obj);
