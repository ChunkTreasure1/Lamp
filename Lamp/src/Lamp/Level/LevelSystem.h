...
public:
  ...
  static LevelEnvironment &GetEnvironment() {
    return m_CurrentLevel->GetEnvironment();
  }
  static void SetCurrentLevel(Ref<Level> level) { m_CurrentLevel = level; }

private:
  LevelSystem() = delete;
  static std::vector<Brush *> LoadBrushes(rapidxml::xml_node<> *pNode,
                                          BrushManager *brushManager);
  static std::vector<Entity *> LoadEntities(rapidxml::xml_node<> *pNode,
                                            EntityManager *entityManager);
  static std::vector<ObjectLayer>
  LoadLayers(rapidxml::xml_node<> *pNode, ObjectLayerManager *objLayerManager);
