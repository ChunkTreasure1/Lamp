...
namespace Lamp
{
	...
	{
		m_Forward.x = glm::sin(glm::radians(m_pEntity->GetPosition().y)) * glm::cos(glm::radians(m_pEntity->GetPosition().x));
		m_Forward.y = glm::sin(glm::radians(-m_pEntity->GetPosition().x));
		m_Forward.z = glm::cos(glm::radians(m_pEntity->GetPosition().x)) * glm::cos(glm::radians(m_pEntity->GetPosition().y));
