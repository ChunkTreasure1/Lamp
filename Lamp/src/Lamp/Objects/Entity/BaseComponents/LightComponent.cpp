...
LightComponent::LightComponent() : EntityComponent("LightComponent") {
  ...
  SetComponentProperties(
      {{PropertyType::Float, "Intensity",
        RegisterData(&m_pPointLight->Intensity)},
...
void LightComponent::OnEvent(Event &e) {
  ...
  dispatcher.Dispatch<EntityPositionChangedEvent>(
      LP_BIND_EVENT_FN(LightComponent::OnPositionChanged));
}

bool LightComponent::OnRender(AppRenderEvent &e) { return false; }

bool LightComponent::OnUpdate(AppUpdateEvent &e) { return false; }
