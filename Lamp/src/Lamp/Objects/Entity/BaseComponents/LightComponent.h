...
public:
    ...
    inline const float GetIntensity() {
        return m_pPointLight->Intensity;
    }
    inline const float GetRadius() {
        return m_pPointLight->Radius;
    }
    inline const float GetFalloff() {
        return m_pPointLight->Falloff;
    }

    inline const glm::vec3& GetColor() {
        return m_pPointLight->Color;
    }
    inline const float GetFarPlane() {
        return m_pPointLight->FarPlane;
    }

    //Setting
    inline void SetIntensity(float val) {
        m_pPointLight->Intensity = val;
    }
    inline void SetColor(const glm::vec3& val) {
        m_pPointLight->Color = val;
    }
...
public:
    static Ref<EntityComponent> Create() {
        return CreateRef<LightComponent>();
    }
