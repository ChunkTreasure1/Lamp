...
Entity *Entity::Create() { return EntityManager::Get()->Create(false); }
...
bool Entity::OnRenderEvent(AppRenderEvent &e) {
  ...
  if (g_pEnv->ShouldRenderGizmos) {
    ...
    m_GizmoShader->UploadMat4(
        "u_ViewProjection", e.GetPassInfo().Camera->GetViewProjectionMatrix());

    glm::vec3 dir =
        glm::normalize(e.GetPassInfo().Camera->GetPosition() - m_Position);

    float angleXZ = std::atan2f(dir.z, dir.x);
    float angleY = -std::asin(dir.y);

    glm::mat4 rotation =
        glm::rotate(glm::mat4(1.f), -angleXZ + glm::radians(90.f),
                    {0.f, 1.f, 0.f}) *
        glm::rotate(glm::mat4(1.f), angleY, {1.f, 0.f, 0.f});

    glm::mat4 model = glm::translate(glm::mat4(1.f), m_Position) * rotation *
