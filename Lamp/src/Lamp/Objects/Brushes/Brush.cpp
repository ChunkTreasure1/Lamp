...
namespace Lamp {
Brush::Brush(Ref<Mesh> model) : m_Mesh(model) { m_Name = "Brush"; }
...
Brush *Brush::Create(const std::filesystem::path &path, const glm::vec3 &pos,
...
Brush *Brush::Get(uint32_t id) {
  if (auto &it = g_pEnv->pLevel->GetBrushes().find(id);
      it != g_pEnv->pLevel->GetBrushes().end()) {
...
bool Brush::OnUpdate(AppUpdateEvent &e) { return false; }
