...
BrushManager::BrushManager() { s_Manager = this; }

BrushManager::~BrushManager() { m_Brushes.clear(); }
...
Brush2D *BrushManager::Create2D(const std::string &path, glm::vec3 pos,
...
Brush *BrushManager::Create(const std::string &path, glm::vec3 pos,
                            glm::vec3 rot, glm::vec3 scale, uint32_t layerId,
...
Brush *BrushManager::GetBrushFromPoint(const glm::vec3 &pos,
                                       const glm::vec3 &origin) {
  Brush *brush = dynamic_cast<Brush *>(
      ObjectLayerManager::Get()->GetObjectFromPoint(pos, origin));
  if (brush) {
    return brush;
  }
...
Brush2D *BrushManager::GetBrush2DFromPoint(const glm::vec3 &pos,
                                           const glm::vec3 &origin) {
  Brush2D *brush = dynamic_cast<Brush2D *>(
      ObjectLayerManager::Get()->GetObjectFromPoint(pos, origin));
  if (brush) {
    return brush;
  }
