...
struct ObjectLayer
{
    ...
    ~ObjectLayer()
    {
        for (int i = 0; i < Objects.size(); i++)
        {
            ...
            //Needs to be done as Object::Destroy removes it self from the object layer
            i--;
        }
...
public:
    ...
    inline void AddLayer(ObjectLayer& layer) {
    ...
    inline void SetLayers(std::vector<ObjectLayer>& layers) {
        m_Layers = layers;
    }
...
public:
    static ObjectLayerManager* Get() {
        return s_ObjectLayerManager;
    }
private:
