...
Ref<Mesh> MeshImporter::ImportMesh(const ImportSettings &settings) {
  ...
  Ref<Mesh> mesh = CreateRef<Mesh>(settings.path.stem().string(), meshes,
                                   materials, boundingBox);

  return mesh;
}

std::vector<Ref<SubMesh>>
MeshImporter::LoadMesh(const ImportSettings &settings,
                       std::map<uint32_t, Ref<Material>> &materials) {
  Assimp::Importer importer;
  std::vector<Ref<SubMesh>> meshes;

  const aiScene *pScene =
      importer.ReadFile(settings.path.string(), s_MeshImportFlags);
  if (!pScene) {
    return meshes;
  }
  ...
  for (uint32_t i = 0; i < pScene->mNumMaterials; i++) {
    materials.emplace(std::make_pair(
        i, CreateRef<Material>(i, pScene->mMaterials[i]->GetName().C_Str())));
  }

  return meshes;
}

void MeshImporter::ProcessNode(aiNode *pNode, const aiScene *pScene,
                               std::vector<Ref<SubMesh>> &meshes, Units units) {
