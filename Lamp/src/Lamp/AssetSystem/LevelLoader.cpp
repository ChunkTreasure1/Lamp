...
void LevelLoader::Save(const Ref<Asset> &asset) const {
  ...
  out << YAML::Key << "level" << YAML::Value;
  {
    ...
    out << YAML::BeginMap;
    {
      ...
      LP_SERIALIZE_PROPERTY(cameraPosition,
                            level->GetEnvironment().CameraPosition, out);
      LP_SERIALIZE_PROPERTY(cameraRotation,
                            level->GetEnvironment().CameraRotation, out);
...
bool LevelLoader::Load(const std::filesystem::path &path,
  ...
  LP_DESERIALIZE_PROPERTY(cameraPosition, level->m_Environment.CameraPosition,
                          envNode, glm::vec3(0.f));
  LP_DESERIALIZE_PROPERTY(cameraRotation, level->m_Environment.CameraRotation,
                          envNode, glm::quat());
  LP_DESERIALIZE_PROPERTY(cameraFOV, level->m_Environment.CameraFOV, envNode,
                          60.f);
  ...
  for (auto &layer : layers) {
    auto layerPath = path.parent_path() /
                     std::filesystem::path("layers/" + layer.Name + ".lyr");
    ...
    for (auto entry : brushesNode) {
      ...
      Brush *brush = Brush::Create(
          g_pEnv->pAssetManager->GetPathFromAssetHandle(meshHandle).string());
    ...
    for (auto entry : entitiesNode) {
      ...
      for (auto compEntry : componentsNode) {
        ...
        for (auto paramEntry : paramsNode) {
          ...
          PropertyType type = ComponentProperties::GetTypeFromString(
              paramEntry["type"].as<std::string>().c_str());

          switch (type) {
          case Lamp::PropertyType::String: {
            std::string *data = GetPropertyData<std::string>(
                paramName, pComp->GetComponentProperties().GetProperties());
          ...
          case Lamp::PropertyType::Bool: {
            bool *data = GetPropertyData<bool>(
                paramName, pComp->GetComponentProperties().GetProperties());
          ...
          case Lamp::PropertyType::Float: {
            float *data = GetPropertyData<float>(
                paramName, pComp->GetComponentProperties().GetProperties());
          ...
          case Lamp::PropertyType::Float2: {
            glm::vec2 *data = GetPropertyData<glm::vec2>(
                paramName, pComp->GetComponentProperties().GetProperties());
          ...
          case Lamp::PropertyType::Float3: {
            glm::vec3 *data = GetPropertyData<glm::vec3>(
                paramName, pComp->GetComponentProperties().GetProperties());
          ...
          case Lamp::PropertyType::Float4: {
            glm::vec4 *data = GetPropertyData<glm::vec4>(
                paramName, pComp->GetComponentProperties().GetProperties());
...
void LevelLoader::SaveEntity(YAML::Emitter &out, const Entity *entity) const {
  ...
  for (const auto &component : entity->GetComponents()) {
    ...
    for (const auto &prop :
         component->GetComponentProperties().GetProperties()) {
      out << YAML::BeginMap;
      LP_SERIALIZE_PROPERTY(param, prop.name, out);
      LP_SERIALIZE_PROPERTY(
          type, ComponentProperties::GetStringFromType(prop.propertyType), out);

      switch (prop.propertyType) {
      case PropertyType::String:
        LP_SERIALIZE_PROPERTY(value, *static_cast<std::string *>(prop.value),
                              out);
        break;
      ...
      case PropertyType::Float2:
        LP_SERIALIZE_PROPERTY(value, *static_cast<glm::vec2 *>(prop.value),
                              out);
        break;

      case PropertyType::Float3:
        LP_SERIALIZE_PROPERTY(value, *static_cast<glm::vec3 *>(prop.value),
                              out);
        break;

      case PropertyType::Float4:
        LP_SERIALIZE_PROPERTY(value, *static_cast<glm::vec4 *>(prop.value),
                              out);
        break;
