...
class Material {
public:
  Material(Ref<Shader> shader, uint32_t id) : m_Index(id) { SetShader(shader); }

  Material(uint32_t index) : m_Index(index), m_Shininess(32.f) {}
  // Setting
  inline void
  SetTextures(std::unordered_map<std::string, Ref<Texture2D>> &map) {
    m_pTextures = map;
  }
