...
void Material::UploadData() {
  ...
  /////Lighting/////
  m_pShader->UploadFloat("u_DirectionalLight.intensity",
                         g_pEnv->DirLight.Intensity);
  m_pShader->UploadFloat3("u_DirectionalLight.direction",
                          glm::normalize(g_pEnv->DirLight.Position));
  ...
  for (auto &light : g_pEnv->pRenderUtils->GetPointLights()) {
    ...
    m_pShader->UploadFloat("u_PointLights[" + v + "].intensity",
                           light->Intensity);
    m_pShader->UploadFloat("u_PointLights[" + v + "].radius", light->Radius);
    m_pShader->UploadFloat("u_PointLights[" + v + "].falloff", light->Falloff);
    m_pShader->UploadFloat("u_PointLights[" + v + "].farPlane",
                           light->FarPlane);

    m_pShader->UploadFloat3("u_PointLights[" + v + "].position",
                            light->ShadowBuffer->GetPosition());
  ...
  for (int i = 4 + lightCount;
       i < m_pShader->GetSpecifications().TextureCount + (4 + lightCount);
       i++) {
    m_pShader->UploadInt(
        "u_Material." +
            m_pShader->GetSpecifications().TextureNames[i - (4 + lightCount)],
        i);
  }
