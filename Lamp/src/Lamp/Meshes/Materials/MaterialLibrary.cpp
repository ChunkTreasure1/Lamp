...
static std::string ToString(const float &var) { return std::to_string(var); }
...
void MaterialLibrary::SaveMaterial(const std::string &path, Material &mat) {
  ...
  pRoot->append_attribute(
      doc.allocate_attribute("name", mat.GetName().c_str()));

  for (auto &tex : mat.GetTextures()) {
    xml_node<> *pTex = doc.allocate_node(node_element, tex.first.c_str());
    pTex->append_attribute(
        doc.allocate_attribute("path", tex.second->GetPath().c_str()));
  ...
  pShader->append_attribute(
      doc.allocate_attribute("name", mat.GetShader()->GetName().c_str()));
  pShader->append_attribute(doc.allocate_attribute(
      "vertex", mat.GetShader()->GetVertexPath().c_str()));
  pShader->append_attribute(doc.allocate_attribute(
      "fragment", mat.GetShader()->GetFragmentPath().c_str()));
...
void MaterialLibrary::LoadMaterials() {
  ...
  for (std::string &path : paths) {
    ...
    std::vector<char> buffer((std::istreambuf_iterator<char>(matFile)),
    ...
    for (auto &texName : mat.GetShader()->GetSpecifications().TextureNames) {
      if (xml_node<> *pTex = pRoot->first_node(texName.c_str())) {
        mat.SetTexture(
            texName, Texture2D::Create(pTex->first_attribute("path")->value()));
      }
    }
