  ...
  if (ImGui::BeginMenuBar()) {
    if (ImGui::BeginMenu("File")) {
      if (ImGui::MenuItem("Save Level")) {
        Lamp::LevelSystem::SaveLevel(
            "engine/levels/" + Lamp::LevelSystem::GetCurrentLevel()->GetName() +
                ".level",
            Lamp::LevelSystem::GetCurrentLevel());
      }
...
void Sandbox2D::RenderPerspective() {
  ...
  ImGui::Begin("Perspective");
  {
    ...
    if (m_PerspectiveSize != *((glm::vec2 *)&perspectivePanelSize)) {
      //	Lamp::Renderer3D::GetFrameBuffer()->Update((uint32_t)perspectivePanelSize.x,
      //(uint32_t)perspectivePanelSize.y);
      m_PerspectiveSize = {perspectivePanelSize.x, perspectivePanelSize.y};

      m_CameraController->UpdateProjection((uint32_t)perspectivePanelSize.x,
                                           (uint32_t)perspectivePanelSize.y);
    }

    uint32_t textureID =
        Lamp::Renderer2D::GetFrameBuffer()->GetColorAttachmentID();
...
void Sandbox2D::RenderProperties() {
  ...
  ImGui::Begin("Properties", &m_InspectiorOpen);
  {
    ...
    if (m_MousePressed && m_PerspectiveHover) {
      ...
      glm::vec3 pos =
          m_CameraController->ScreenToWorldCoords(mousePos, m_PerspectiveSize);
