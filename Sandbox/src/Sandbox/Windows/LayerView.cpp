...
LayerView::LayerView(std::string_view name) : BaseWindow(name) {
  m_entityIcon = ResourceCache::GetAsset<Texture2D>(
      "engine/textures/gizmos/gizmoEntity.png");
  m_brushIcon = ResourceCache::GetAsset<Texture2D>(
      "engine/textures/ui/AssetIcons/iconMesh.png");

  m_visibleIconV = ResourceCache::GetAsset<Texture2D>(
      "engine/textures/ui/layerView/visible_v.png");
  m_visibleIconN = ResourceCache::GetAsset<Texture2D>(
      "engine/textures/ui/layerView/visible_n.png");

  m_lockedIcon = ResourceCache::GetAsset<Texture2D>(
      "engine/textures/ui/layerView/locked.png");
  m_unlockedIcon = ResourceCache::GetAsset<Texture2D>(
      "engine/textures/ui/layerView/unlocked.png");
}

void LayerView::OnEvent(Lamp::Event &e) {
  EventDispatcher dispatcher(e);
  dispatcher.Dispatch<ImGuiUpdateEvent>(
      LP_BIND_EVENT_FN(LayerView::UpdateImGui));
...
bool LayerView::UpdateImGui(Lamp::ImGuiUpdateEvent &e) {
  ...
  for (auto &layer : g_pEnv->pLevel->GetLayers()) {
    ...
    ImGui::SameLine();
    std::string id = m_renamingLayer == layer.ID
    ...
    CollapsingHeaderAddons(currentRightClick,
                           layer.Name + "###layer" + std::to_string(layer.ID),
                           itemMenuOpen, idMenuOpen, layer.ID);
    ...
    if (open) {
      ...
      UI::ScopedColor hovered(ImGuiCol_ButtonHovered,
                              {0.3f, 0.305f, 0.31f, 0.5f});
      UI::ScopedColor active(ImGuiCol_ButtonActive,
                             {0.5f, 0.505f, 0.51f, 0.5f});
      ...
      if (ImGui::BeginTable(id.c_str(), 3,
        ...
        for (auto obj : layer.Objects) {
          ...
          uint32_t visibleIcon = obj->GetIsActive() ? m_visibleIconV->GetID()
                                                    : m_visibleIconN->GetID();

          if (ImGui::ImageButton((ImTextureID)(visibleIcon),
                                 {imageSize, imageSize}, {0.f, 1.f}, {1.f, 0.f},
                                 0)) {
            obj->SetIsActive(!obj->GetIsActive());
          }
          ImGui::SameLine();

          uint32_t lockedIcon = obj->GetIsFrozen() ? m_lockedIcon->GetID()
                                                   : m_unlockedIcon->GetID();
          if (ImGui::ImageButton((ImTextureID)(lockedIcon),
                                 {imageSize, imageSize}, {0.f, 1.f}, {1.f, 0.f},
                                 0)) {
          ...
          std::string id = obj->GetName() + "##" + obj->GetName() +
          ...
          if (ImGui::BeginDragDropSource()) {
            ...
            ImGui::SetDragDropPayload("LAYER_OBJECT", values,
                                      sizeof(uint32_t) * 2, ImGuiCond_Once);
  ...
  if (ImGui::IsMouseClicked(ImGuiMouseButton_Right) &&
      ImGui::IsWindowHovered() && !itemMenuOpen) {
...
void LayerView::UpdateMainClickMenu() {
  if (ImGui::BeginPopup("LV_Main")) {
    if (ImGui::Selectable("Add")) {
      g_pEnv->pLevel->AddLayer(ObjectLayer(
          "New layer", (uint32_t)g_pEnv->pLevel->GetLayers().size(), true));
...
void LayerView::UpdateItemClickMenu(const std::string &id, bool &open,
                                    uint32_t &layerId) {
...
void LayerView::CollapsingHeaderAddons(std::string &currRightClick,
                                       const std::string &id, bool &open,
                                       uint32_t &layerId,
                                       const uint32_t &currId) {
  ...
  if (ImGui::BeginDragDropTarget()) {
    if (const ImGuiPayload *pPayload =
            ImGui::AcceptDragDropPayload("LAYER_OBJECT")) {
