...
MaterialEditor::MaterialEditor(std::string_view name) : BaseWindow(name) {
...
void MaterialEditor::OnEvent(Lamp::Event &e) {
  ...
  dispatcher.Dispatch<AppUpdateEvent>(
      LP_BIND_EVENT_FN(MaterialEditor::OnUpdate));
  dispatcher.Dispatch<ImGuiUpdateEvent>(
      LP_BIND_EVENT_FN(MaterialEditor::OnUpdateImGui));
  dispatcher.Dispatch<KeyPressedEvent>(
      LP_BIND_EVENT_FN(MaterialEditor::OnKeyPressed));
}

bool MaterialEditor::OnUpdate(Lamp::AppUpdateEvent &e) { return false; }
...
void MaterialEditor::UpdateMaterialView() {
  ...
  if (m_renderGraph->GetSpecification().endNode->framebuffer) {
    ...
    ImGui::Image((ImTextureID)textureID, ImVec2{panelSize.x, panelSize.y},
  ...
  if (m_materialModel) {
    for (const auto &mesh : m_materialModel->GetSubMeshes()) {
      Renderer3D::SubmitMesh(
          glm::mat4(1.f), mesh,
          m_materialModel->GetMaterials()[mesh->GetMaterialIndex()]);
    }
  }
...
void MaterialEditor::CreateNewMaterial() {
  ...
  ofs << "	<albedo "
         "path=\"engine/textures/default/defaultTexture.png\"/>\n";
  ofs << "	<normal "
         "path=\"engine/textures/default/defaultTexture.png\"/>\n";
  ofs << "	<mro path=\"engine/textures/default/defaultTexture.png\"/>\n";
  ofs << "	<Shader name=\"testPbr\" "
         "vertex=\"engine/shaders/3d/testPbr_vs.glsl\" "
         "fragment=\"engine/shaders/3d/testPbr_fs.glsl\"/>\n";
  ofs << "</Material>";
  ofs.close();
}
} // namespace Sandbox