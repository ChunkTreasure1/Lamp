...
Sandbox::Sandbox()
    : Layer("Sandbox"), m_DockspaceID(0),
      m_PhysicsIcon("engine/textures/ui/physicsIcon/LampPhysicsAnim1.png", 30) {
  g_pEnv->IsEditor = true;
  m_IconPlay =
      ResourceCache::GetAsset<Texture2D>("engine/textures/ui/PlayIcon.png");
  m_IconStop =
      ResourceCache::GetAsset<Texture2D>("engine/textures/ui/StopIcon.png");

  m_pLevel =
      ResourceCache::GetAsset<Level>("assets/levels/testLevel/data.level");
  ...
  Renderer::SetRenderGraph(
      ResourceCache::GetAsset<RenderGraph>("assets/testGraph.rendergraph"));
...
bool Sandbox::OnUpdate(AppUpdateEvent &e) {
  ...
  if (Input::IsMouseButtonPressed(1) &&
      (m_PerspectiveHover || m_RightMousePressed)) {
...

  {
    ...
    case SceneState::Edit: {
      g_pEnv->pLevel->UpdateEditor(
          e.GetTimestep(),
          m_SandboxController->GetCameraController()->GetCamera());
      break;
    }
    ...
    case SceneState::Simulating: {
      g_pEnv->pLevel->UpdateSimulation(
          e.GetTimestep(),
          m_SandboxController->GetCameraController()->GetCamera());
    }
...
void Sandbox::OnEvent(Event &e) {
  ...
  dispatcher.Dispatch<WindowCloseEvent>(
      LP_BIND_EVENT_FN(Sandbox::OnWindowClose));
  ...
  dispatcher.Dispatch<EditorViewportSizeChangedEvent>(
      LP_BIND_EVENT_FN(Sandbox::OnViewportSizeChanged));
  dispatcher.Dispatch<EditorObjectSelectedEvent>(
      LP_BIND_EVENT_FN(Sandbox::OnObjectSelected));
}
