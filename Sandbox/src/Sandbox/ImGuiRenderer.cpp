...
void Sandbox::UpdatePerspective() {
  ...
  ImGui::Begin("Perspective");
  {
    ...
    m_SandboxController->GetCameraController()->SetControlsEnabled(
        m_PerspectiveHover);
    ...
    m_PerspectiveBounds[0] = {viewportMinRegion.x + viewportOffset.x,
                              viewportMinRegion.y + viewportOffset.y};
    m_PerspectiveBounds[1] = {viewportMaxRegion.x + viewportOffset.x,
    ...
    if (m_PerspectiveSize != *((glm::vec2 *)&perspectivePanelSize)) {
      ...
      Lamp::EditorViewportSizeChangedEvent e((uint32_t)perspectivePanelSize.x,
                                             (uint32_t)perspectivePanelSize.y);
      OnEvent(e);
    ...
    if (Renderer::GetRenderGraph()) {
      ...
      if (renderGraph->GetSpecification().endNode->framebuffer) {
        textureID = renderGraph->GetSpecification()
                        .endNode->framebuffer->GetColorAttachmentID(0);
        m_SelectionBuffer =
            renderGraph->GetSpecification().endNode->framebuffer;
      }
    }
    ImGui::Image((void *)(uint64_t)textureID,
                 ImVec2{m_PerspectiveSize.x, m_PerspectiveSize.y}, ImVec2{0, 1},
                 ImVec2{1, 0});
    ...
    if (ImGui::BeginDragDropTarget()) {
      if (const ImGuiPayload *pPayload =
        ...
        std::filesystem::path path =
            std::filesystem::path("assets") / std::filesystem::path(wPath);
      ...
      if (const ImGuiPayload *pPayload =
              ImGui::AcceptDragDropPayload("BRUSH_ITEM")) {
  ...
  if (m_pSelectedObject && m_SceneState != SceneState::Play) {
    ...
    ImGuizmo::SetRect(m_PerspectiveBounds[0].x, m_PerspectiveBounds[0].y,
                      m_PerspectiveBounds[1].x - m_PerspectiveBounds[0].x,
                      m_PerspectiveBounds[1].y - m_PerspectiveBounds[0].y);
    ...
    ImGuizmo::Manipulate(
        glm::value_ptr(m_SandboxController->GetCameraController()
                           ->GetCamera()
                           ->GetViewMatrix()),
        glm::value_ptr(m_SandboxController->GetCameraController()
                           ->GetCamera()
...
void Sandbox::UpdateProperties() {
  ...
  ImGui::Begin("Properties", &m_InspectiorOpen);
  {
    ...
    glm::vec2 mousePos =
        glm::vec2(io.MouseClickedPos->x, io.MouseClickedPos->y);
    glm::vec2 windowPos;
    glm::vec2 windowSize;
    ...
    ImGui::Begin("Perspective");
    {
      windowPos = glm::vec2(ImGui::GetCursorScreenPos().x,
    ...
    if (m_MousePressed && perspHover && !ImGuizmo::IsOver() &&
      ...
      glm::vec2 perspectiveSize =
          m_PerspectiveBounds[1] - m_PerspectiveBounds[0];
      my = perspectiveSize.y - my;

      int mouseX = (int)mx;
      int mouseY = (int)my;

      if (mouseX >= 0 && mouseY >= 0 && mouseX < (int)perspectiveSize.x &&
          mouseY < (int)perspectiveSize.y) {
    ...
    if (auto pEnt = dynamic_cast<Lamp::Entity *>(m_pSelectedObject)) {
      ...
      if (UI::TreeNodeFramed("Transform")) {
        ...
        UI::PropertyAxisColor("Position",
                              const_cast<glm::vec3 &>(pEnt->GetPosition()));
        ...
        UI::PropertyAxisColor("Scale",
                              const_cast<glm::vec3 &>(pEnt->GetScale()), 1.f);

        UI::EndProperties();
        UI::TreeNodePop();
      }

      std::string graphButtonString =
          pEnt->GetGraphKeyGraph() ? "Open Graph" : "Create Graph";
      ...
      if (UI::TreeNodeFramed("Transform")) {
        ...
        UI::PropertyAxisColor("Position",
                              const_cast<glm::vec3 &>(pBrush->GetPosition()));
        ...
        UI::PropertyAxisColor("Scale",
                              const_cast<glm::vec3 &>(pBrush->GetScale()));
    ...
    for (auto &key : Lamp::ComponentRegistry::s_Methods()) {
      if (ImGui::MenuItem(key.first.c_str())) {
        if (auto pEnt = dynamic_cast<Lamp::Entity *>(m_pSelectedObject)) {
          ...
          Ref<Lamp::EntityComponent> comp =
              Lamp::ComponentRegistry::Create(key.first.c_str());
          if (comp != nullptr) {
...
void Sandbox::UpdateCreateTool() {
  ...
  if (brushListOpen) {
    ...
    if (ImGui::Button("Create")) {
                      ...
                      {
                              m_pSelectedObject =
         Lamp::Brush::Create(m_SelectedFile.GetPath());
                      }*/
...
bool Sandbox::DrawComponent(Lamp::EntityComponent *ptr) {
  ...
  if (!open) {
    UI::SameLine(availRegion - buttonSize * 0.5f +
                 GImGui->Style.FramePadding.x * 0.5f);
  } else {
    UI::SameLine(availRegion + buttonSize * 0.5f +
                 GImGui->Style.FramePadding.x * 0.5f);
  }
  ...
  if (open) {
    ...
    for (auto &prop : ptr->GetComponentProperties().GetProperties()) {
      ...
      case Lamp::PropertyType::Int:
        propertyChanged =
            UI::Property(prop.name, *static_cast<int *>(prop.value));
        break;
      case Lamp::PropertyType::Bool:
        propertyChanged =
            UI::Property(prop.name, *static_cast<bool *>(prop.value));
        break;
      case Lamp::PropertyType::Float:
        propertyChanged =
            UI::Property(prop.name, *static_cast<float *>(prop.value));
        break;
      case Lamp::PropertyType::Float2:
        propertyChanged =
            UI::Property(prop.name, *static_cast<glm::vec2 *>(prop.value));
        break;
      case Lamp::PropertyType::Float3:
        propertyChanged =
            UI::Property(prop.name, *static_cast<glm::vec3 *>(prop.value));
        break;
      case Lamp::PropertyType::Float4:
        propertyChanged =
            UI::Property(prop.name, *static_cast<glm::vec4 *>(prop.value));
        break;
      case Lamp::PropertyType::String:
        propertyChanged =
            UI::Property(prop.name, *static_cast<std::string *>(prop.value));
        break;
      case Lamp::PropertyType::Path:
        propertyChanged = UI::Property(
            prop.name,
            std::filesystem::path(*static_cast<std::string *>(prop.value)));
        break;
      case Lamp::PropertyType::Color3:
        propertyChanged = UI::Property(
            prop.name, *static_cast<glm::vec3 *>(prop.value), false);
        break;
      case Lamp::PropertyType::Color4:
        propertyChanged = UI::Property(
            prop.name, *static_cast<glm::vec4 *>(prop.value), true);
        break;
      }
...
void Sandbox::UpdateRenderingSettings() {
  ...
  std::string graphPath = Renderer::GetRenderGraph()
                              ? Renderer::GetRenderGraph()->Path.string()
                              : "";
  ...
  if (ImGui::Button("Open")) {
    graphPath =
        FileDialogs::OpenFile("RenderGraph (*.rendergraph)\0*.rendergraph\0");
    Renderer::SetRenderGraph(ResourceCache::GetAsset<RenderGraph>(graphPath));
  ...
  if (ImGui::Button("Reload")) {
    if (Renderer::GetRenderGraph()) {
      Renderer::SetRenderGraph(ResourceCache::ReloadAsset<RenderGraph>(
          std::dynamic_pointer_cast<Asset>(Renderer::GetRenderGraph())));
      Renderer::GetRenderGraph()->Start();
    }
  }
...
void Sandbox::UpdateToolbar() {
  ...
  if (ImGui::ImageButton((ImTextureID)playIcon->GetID(), {size, size},
                         {0.f, 0.f}, {1.f, 1.f}, 0)) {
  ...
  if (ImGui::ImageButtonAnimated((ImTextureID)physicsId,
                                 (ImTextureID)physicsIcon->GetID(),
                                 {size, size}, {0.f, 0.f}, {1.f, 1.f}, 0)) {
    if (m_SceneState == SceneState::Edit) {
      OnSimulationStart();
      m_PhysicsIcon.Play();
...
void Sandbox::UpdateStatistics() {
  ...
  ImGui::Text("Frame time: %f",
              Application::Get().GetFrameTime().GetFrameTime() * 1000);
  ImGui::Text("Frames per second: %f",
              Application::Get().GetFrameTime().GetFramesPerSecond());
  ...
  if (ImGui::BeginMenuBar()) {
    ...
    if (ImGui::BeginMenu("Tools")) {
      ...
      for (auto pWindow : m_pWindows) {
        ImGui::MenuItem(pWindow->GetLabel().c_str(), NULL,
                        &pWindow->GetIsOpen());
      }
    ...
    if (ImGui::BeginMenu("Play")) {
      ...
      ImGui::MenuItem("Play physics", NULL,
                      &Lamp::Application::Get().GetIsSimulating());
