...
ModelImporter::ModelImporter() {
...

  {
    ...
    mainBuffer.Attachments = {{FramebufferTextureFormat::RGBA8,
...
void ModelImporter::UpdatePerspective() {
  ...
  ImGui::Begin("Import Perspective");
  {
    ...
    ImGui::Image((void *)(uint64_t)textureID, ImVec2{panelSize.x, panelSize.y},
...
void ModelImporter::UpdateProperties() {
  ...
  if (ImGui::Button("Load##fbx")) {
    ...
    if (path != "" && std::filesystem::exists(path)) {
      ...
      for (auto &tex : m_pModelToImport->GetMaterial().GetTextures()) {
        tex.second = Lamp::Texture2D::Create(
  ...
  if (ImGui::Button("Save")) {
    ...
    if (m_MaterialName.empty()) {
      m_MaterialName =
          savePath.substr(savePath.find_last_of('\\') + 1, savePath.size() - 1);
    }
    ...
    if (savePath != "") {
      ...
      if (MaterialLibrary::GetMaterial(
              m_pModelToImport->GetMaterial().GetName())
              .GetIndex() == -1) {
...
void ModelImporter::UpdateMaterial() {
  ...
  if (m_pModelToImport.get() != nullptr) {
    for (int i = 0; i < shaders.size(); i++) {
      if (m_pModelToImport->GetMaterial().GetShader()->GetName() ==
          shaders[i]) {
        selectedItem = i;
      }
    }
  ...
  if (m_pModelToImport.get() != nullptr) {
    if (m_pModelToImport->GetMaterial().GetShader() !=
        ShaderLibrary::GetShader(shaders[selectedItem])) {
      paths.clear();

      m_pModelToImport->GetMaterial().SetShader(
          ShaderLibrary::GetShader(shaders[selectedItem]));

      for (auto &tex : m_pModelToImport->GetMaterial().GetTextures()) {
        tex.second =
            Texture2D::Create("engine/textures/default/defaultTexture.png");
      }
    }
  }
  ...
  if (m_pModelToImport != nullptr) {
    ...
    for (auto &tex : m_pModelToImport->GetMaterial().GetTextures()) {
      if (ImGui::Button((std::string("Load##") + tex.first).c_str())) {
        paths[tex.first] = Lamp::FileDialogs::OpenFile(
            "Texture (*.png ...)\0*.png\0*.PNG\0*.jpg\0");
      }
      if (paths[tex.first] != "") {
        m_pModelToImport->GetMaterial().SetTexture(
            tex.first, Lamp::Texture2D::Create(paths[tex.first]));
      } else {
        m_pModelToImport->GetMaterial().SetTexture(
            tex.first, Lamp::Texture2D::Create(
                           "engine/textures/default/defaultTexture.png"));
      }
