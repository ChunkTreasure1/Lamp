...
void Sandbox3D::UpdatePerspective() {
  ...
  ImGui::Begin("Perspective");
  {
    ...
    if (m_PerspectiveSize != *((glm::vec2 *)&perspectivePanelSize)) {
      m_SandboxBuffer->Resize((uint32_t)perspectivePanelSize.x,
...
void Sandbox3D::UpdateProperties() {
  ...
  ImGui::Begin("Properties", &m_InspectiorOpen);
  {
    ...
    if (m_MousePressed && perspHover && !ImGuizmo::IsOver()) {
      ...
      if (mouseX >= 0 && mouseY >= 0 && mouseX < (int)m_PerspectiveSize.x &&
          mouseY < (int)m_PerspectiveSize.y) {
        ...
        m_pSelectedObject =
            Lamp::ObjectLayerManager::Get()->GetObjectFromId(pixelData);
        if (m_pSelectedObject) {
...
void Sandbox3D::UpdateLevelSettings() {
  ...
  if (ImGui::CollapsingHeader("Environment")) {
    ImGui::DragFloat3("Sun direction",
                      glm::value_ptr(g_pEnv->DirLight.Position));
