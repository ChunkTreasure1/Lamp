...
void Sandbox3D::UpdatePerspective() {
  ...
  ImGui::Begin("Perspective");
  {
    ...
    m_PerspectiveHover =
        ...
        !m_ModelImporter->GetIsOpen() && ImGui::IsWindowHovered();
    ...
    if (m_PerspectiveSize != *((glm::vec2 *)&perspectivePanelSize)) {
      Lamp::Renderer3D::GetFrameBuffer()->Update(
          (uint32_t)perspectivePanelSize.x, (uint32_t)perspectivePanelSize.y);
      m_PerspectiveSize = {perspectivePanelSize.x, perspectivePanelSize.y};

      m_SandboxController->GetCameraController()->UpdateProjection(
          (uint32_t)perspectivePanelSize.x, (uint32_t)perspectivePanelSize.y);
    }

    uint32_t textureID =
        Lamp::Renderer3D::GetFrameBuffer()->GetColorAttachment();
    ...
    std::string frameInfo =
        "FrameTime: " +
        std::to_string(Lamp::Application::Get().GetFrameTime().GetFrameTime()) +
        ". FPS: " +
        std::to_string(
            Lamp::Application::Get().GetFrameTime().GetFramesPerSecond()) +
        ". Using VSync: " +
        std::to_string(Lamp::Application::Get().GetWindow().GetIsVSync());
  ...
  if (m_pSelectedObject) {
    ...
    ImGuizmo::SetRect(perspectivePos.x, perspectivePos.y, m_PerspectiveSize.x,
                      m_PerspectiveSize.y);
    ImGuizmo::Manipulate(
    ...
    if (m_pSelectedObject->GetModelMatrix() != lastTrans &&
        !ImGuizmo::IsDragging()) {
      Command cmd;
  ...
  ImGui::Begin("Asset Browser", &m_AssetBrowserOpen);
  {
    // Asset browser
    {
      ImGui::BeginChild("Browser",
                        ImVec2(ImGui::GetWindowContentRegionWidth() * 0.12f,
                               ImGui::GetWindowSize().y * 0.85f),
                        true);
      {
      ...
      ImGui::BeginChild("Viewer",
                        ImVec2(ImGui::GetWindowSize().y * 0.85f,
                               ImGui::GetWindowSize().y * 0.85f),
                        true);
      {
        if (m_SelectedFile.GetFileType() == Lamp::FileType::Texture) {
          ...
          ImGui::Image((void *)(uint64_t)selected->GetID(),
                       ImVec2(ImGui::GetWindowSize().y * 0.9f,
                              ImGui::GetWindowSize().y * 0.9f));
        }
      }
      ImGui::EndChild();
    }
...
void Sandbox3D::UpdateProperties() {
  ...
  ImGui::Begin("Properties", &m_InspectiorOpen);
  {
    ...
    if (m_MousePressed && perspHover && !ImGuizmo::IsOver()) {
      ...
      m_pSelectedObject = Lamp::ObjectLayerManager::Get()->GetObjectFromPoint(
          m_SandboxController->GetCameraController()->ScreenToWorldCoords(
              mousePos, m_PerspectiveSize),
          m_SandboxController->GetCameraController()->GetPosition());
    ...
    if (auto pEnt = dynamic_cast<Lamp::Entity *>(m_pSelectedObject)) {
      ...
      if (ImGui::CollapsingHeader("Material")) {
        ...
        if (n != pBrush->GetModel()->GetMaterial().GetName()) {
          if (auto mat = Lamp::MaterialLibrary::GetMaterial(n);
              mat.GetIndex() != -1) {
            pBrush->GetModel()->SetMaterial(
                Lamp::MaterialLibrary::GetMaterial(n));
          }
        }
      }
    }
...
void Sandbox3D::UpdateLayerView() {
  ...
  for (Lamp::ObjectLayer &layer :
       Lamp::ObjectLayerManager::Get()->GetLayers()) {
    ...
    if (ImGui::TreeNode(layer.Name.c_str())) {
      ...
      for (int i = 0; i < layer.Objects.size(); i++) {
        ...
        ImGui::TreeNodeEx((void *)(intptr_t)startId, nodeFlags,
                          layer.Objects[i]->GetName().c_str());
...
void Sandbox3D::UpdateLevelSettings() {
  ...
  ImGui::ColorEdit3(
      "Global Ambient",
      glm::value_ptr(Lamp::LevelSystem::GetEnvironment().GlobalAmbient));
