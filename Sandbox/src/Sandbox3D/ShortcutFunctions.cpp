...
void Sandbox3D::SaveLevelAs() {
  std::string filepath =
      Lamp::FileDialogs::SaveFile("Lamp Level (*.level)\0*.level\0");
  if (!filepath.empty()) {
    Lamp::LevelSystem::SaveLevel(filepath,
...
void Sandbox3D::OpenLevel() {
  std::string filepath =
      Lamp::FileDialogs::OpenFile("Lamp Level (*.level)\0*.level\0");
  if (!filepath.empty()) {
...
void Sandbox3D::Undo() {
  ...
  case Cmd::Transform: {
    if (auto *pObj =
            static_cast<Lamp::Object *>(m_PerspecticeCommands.front().object)) {
      pObj->SetModelMatrix(
          glm::make_mat4((float *)m_PerspecticeCommands.front().lastData));
...
void Sandbox3D::Redo() {
  ...
  case Cmd::Transform: {
    if (auto *pObj = static_cast<Lamp::Object *>(
            m_PerspecticeCommands.redo_top().object)) {
      pObj->SetModelMatrix(
          glm::make_mat4((float *)m_PerspecticeCommands.redo_top().lastData));
  ...
  case Cmd::Selection: {
    m_pSelectedObject =
        (Lamp::Object *)m_PerspecticeCommands.redo_top().lastData;
    m_PerspecticeCommands.redo_pop();
    break;
  }
  }
