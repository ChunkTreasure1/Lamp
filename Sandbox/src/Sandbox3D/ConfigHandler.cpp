...
bool Sandbox3D::OnWindowClose(Lamp::WindowCloseEvent &e) {
  ...
  char *pSize = doc.allocate_string(
      ToString(glm::vec2(Lamp::Application::Get().GetWindow().GetWidth(),
                         Lamp::Application::Get().GetWindow().GetHeight()))
          .c_str());
  ...
  pWindowsOpen->append_attribute(
      doc.allocate_attribute("AssetBrowser", pAssetB));
  ...
  char *pModelImp =
      doc.allocate_string(ToString(m_ModelImporter->GetIsOpen()).c_str());
  pWindowsOpen->append_attribute(
      doc.allocate_attribute("ModelImporter", pModelImp));
...
void Sandbox3D::SetupFromConfig() {
  ...
  if (xml_node<> *pWindowsOpen = pRootNode->first_node("WindowsOpen")) {
    GetValue(pWindowsOpen->first_attribute("Perspective")->value(),
             m_PerspectiveOpen);
    GetValue(pWindowsOpen->first_attribute("AssetBrowser")->value(),
             m_AssetBrowserOpen);
    GetValue(pWindowsOpen->first_attribute("Inspector")->value(),
             m_InspectiorOpen);
    GetValue(pWindowsOpen->first_attribute("ModelImporter")->value(),
             m_ModelImporter->GetIsOpen());
    GetValue(pWindowsOpen->first_attribute("LayerView")->value(),
             m_LayerViewOpen);
    GetValue(pWindowsOpen->first_attribute("CreateTool")->value(),
             m_CreateToolOpen);
