...
mat2x3 CalculateDirectionalLight(DirectionalLight light, vec3 dirToCamera, vec3 normal, vec3 baseReflectivity, vec3 albedo, float metallic, float roughness, uint lightIndex)
{
    float shadow = 0.0;
    ...
    vec3 lightDiff = kD * albedo / PI;
    vec3 lightSpec = specular;

    return mat2x3(lightDiff, lightSpec);
}

mat2x3 CalculatePointLight(PointLight light, vec3 dirToCamera, vec3 normal, vec3 baseReflectivity, float metallic, float roughness, vec3 albedo, vec3 fragPos)
{
    //Radiance
    ...
    vec3 lightDiff = diffuse * radiance * NdotL;
    vec3 lightSpec = specular * radiance * NdotL;
...
mat2x3 CalculatePointLights(vec3 dirToCamera, vec3 normal, vec3 baseReflectivity, vec3 albedo, float metallic, float roughness)
{
    mat2x3 result = mat2x3(0.0);
...
mat2x3 CalculateDirectionalLights(vec3 dirToCamera, vec3 normal, vec3 baseReflectivity, vec3 albedo, float metallic, float roughness)
{
    mat2x3 lightAccumulation = mat2x3(0.0);
...
mat2x3 CalculateAmbience(vec3 dirToCamera, vec3 normal, vec3 baseReflectivity, vec3 albedo, float metallic, float roughness)
{
...
void main()
{
    ...
    {
        normal = normalize(u_MeshData.normalColor);
    }
    ...
    vec3 baseReflectivity = translucency * mix(globalDielectricBase, albedo.xyz, metallic);
    mat2x3 lightAccumulation = mat2x3(0.0);
    ...
    o_Diffuse = vec4(lightAccumulation[0], 1.0);
    o_Specular = vec4(lightAccumulation[1], 1.0);
}