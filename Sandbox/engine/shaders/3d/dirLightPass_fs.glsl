...
vec2 CalcTexCoord()
{
	return gl_FragCoord.xy / u_BufferSize;
}
...
void main()
{
	...
	vec3 fragPos = texture(u_GBuffer.positionAO, texCoords).rbg;
	vec3 normal = texture(u_GBuffer.normalMetallic, texCoords).rgb;
	vec3 albedo = texture(u_GBuffer.albedoRoughness, texCoords).rgb;

	normal = normalize(normal);
	float ao = 0;

	float metallic = texture(u_GBuffer.normalMetallic, texCoords).a;
	float roughness = texture(u_GBuffer.albedoRoughness, texCoords).a;
